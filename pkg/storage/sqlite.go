...
func (s *Store) ListProjects() ([]string, error) {
	query := `SELECT DISTINCT project FROM secrets ORDER BY project`
...
func (s *Store) CreateToken(token, name string) error {
	_, err := s.db.Exec("INSERT INTO tokens (token, name) VALUES (?, ?)", token, name)
	return err
}
